...
enum AppMode { MODE_EDITOR = 0, MODE_SPRITE_LIBRARY };

enum ActiveInput { INPUT_NONE = 0, INPUT_PROJECT_NAME, INPUT_SPRITE_NAME, INPUT_X, INPUT_Y, INPUT_DIRECTION, INPUT_SIZE, INPUT_BLOCK_FIELD, INPUT_VAR_MODAL };
...
struct Sprite {
    ...
    // EACH SPRITE OWNS ITS OWN BLOCKS!
...
struct AppState {
    ...
    Tab current_tab; bool start_hover, stop_hover, running;
    
    // ---> NEW: MULTI-SPRITE MEMORY <---
    AppMode mode;
