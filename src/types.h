...
enum AppMode { MODE_EDITOR = 0, MODE_SPRITE_LIBRARY, MODE_BACKDROP_LIBRARY, MODE_EXTENSION_LIBRARY };
...
enum ActiveInput {
    ...
    INPUT_SOUND_NAME, INPUT_SOUND_VOLUME, INPUT_PEN_COLOR_PICKER,
    INPUT_COSTUME_NAME, INPUT_COSTUME_TEXT
};
...
enum LooksBlockType { LB_SAY_FOR = 0, LB_SAY, LB_THINK_FOR, LB_THINK, LB_SWITCH_COSTUME_TO, LB_NEXT_COSTUME, LB_SWITCH_BACKDROP_TO, LB_NEXT_BACKDROP, LB_CHANGE_SIZE_BY, LB_SET_SIZE_TO, LB_SHOW, LB_HIDE, LB_GO_TO_LAYER, LB_GO_LAYERS, LB_SIZE, LB_BACKDROP_NUM_NAME, LB_COSTUME_NUM_NAME };
...
enum SensingBlockType { SENSB_TOUCHING = 0, SENSB_ASK_AND_WAIT, SENSB_KEY_PRESSED, SENSB_MOUSE_DOWN, SENSB_SET_DRAG_MODE, SENSB_ANSWER, SENSB_DISTANCE_TO, SENSB_COUNT };
...
enum PenBlockType { PB_ERASE_ALL = 0, PB_STAMP, PB_PEN_DOWN, PB_PEN_UP, PB_SET_COLOR_TO_PICKER, PB_CHANGE_ATTRIB_BY, PB_SET_ATTRIB_TO, PB_CHANGE_SIZE_BY, PB_SET_SIZE_TO };
enum BlockKind { BK_MOTION = 0, BK_LOOKS = 1, BK_SOUND = 2, BK_EVENTS = 3, BK_CONTROL = 4, BK_SENSING = 5, BK_OPERATORS = 6, BK_VARIABLES = 7, BK_PEN = 8 };
...
enum EditTool { TOOL_POINTER = 0, TOOL_BRUSH, TOOL_ERASER, TOOL_TEXT, TOOL_FILL, TOOL_RECT, TOOL_CIRCLE };
enum ShapeType { SHAPE_RECT = 0, SHAPE_CIRCLE, SHAPE_TEXT };
...
struct Sprite {
    ...
    static int get_next_layer() { static int l = 0; return l++; }
};
...
inline std::string copy_asset_to_project(std::string proj_name, std::string original_path) {
    if (proj_name.empty()) proj_name = "Untitled";
...
// ---> FIXED: IRONCLAD SAFETY SHIELD <---
inline void delete_asset_from_project(std::string path) {
    if (path.empty()) return;
    
    // 1. NEVER delete files from the default 'assets' folder!
    if (path.find("assets/") == 0 || path.find("assets\\") == 0) {
        return; 
    }
    
    // 2. Only delete files if they are actually inside a user's "Projects" folder
